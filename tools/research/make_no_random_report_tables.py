...
def fmt(x, ndigits=4):
    y = safe_float(x)
    return "" if y is None else round(y, ndigits)
...
def build_combined_corruption_severity_table(rows, corruptions, max_severity):
    table = []
    ...
    for model, model_rows in by_model(rows).items():
        ...
        row["Clean_AP"] = fmt(clean_ap)
        row["Clean_AP50"] = fmt(clean_ap50)
        row["Clean_AP75"] = fmt(clean_ap75)

        for corruption in corruptions:
            cname = corr_name(corruption).replace(" ", "")
            ...
            for severity in range(1, max_severity + 1):
                ...
                row[f"{cname}_S{severity}_AP"] = fmt(
                    mean(safe_float(r.get("mask_AP")) for r in matched)
                )
                row[f"{cname}_S{severity}_AP50"] = fmt(
                    mean(safe_float(r.get("mask_AP50")) for r in matched)
                )
                row[f"{cname}_S{severity}_AP75"] = fmt(
                    mean(safe_float(r.get("mask_AP75")) for r in matched)
                )
...
def build_corruption_severity_metric_table(rows, corruption, metric_key, max_severity):
    table = []
    ...
    for model, model_rows in by_model(rows).items():
        ...
        for severity in range(1, max_severity + 1):
            ...
            row[f"S{severity}"] = fmt(
                mean(safe_float(r.get(metric_key)) for r in matched)
            )
...
def parse_training_time(run_dir):
    ...
    for json_log in json_logs:
        try:
            for line in json_log.read_text(
                encoding="utf-8",
                errors="ignore",
            ).splitlines():
                line = line.strip()
    ...
    best_checkpoints = sorted(run_dir.glob("best_*.pth"))
    log_files = sorted(run_dir.glob("*.log"))

    return {
        "run_dir": str(run_dir),
        "best_checkpoint": best_checkpoints[-1].name if best_checkpoints else "",
...
def plot_heatmap(
    ...
    fig_w = max(8.5, len(cols) * 1.35 + 4.5)
    ...
    if "Clean" in cols and len(cols) > 1:
        clean_index = cols.index("Clean")
        ax.axvline(clean_index + 0.5, color="white", linewidth=5.0)
...
def build_metric_heatmap_table_from_combined(combined_table, corruptions, max_severity, metric_suffix):
    table = []

    clean_source_col = f"Clean_{metric_suffix}"

    for row in combined_table:
        new_row = {
            "Model": row["Model"],
            "Clean": row.get(clean_source_col, ""),
        }
...
def plot_combined_corruption_severity_heatmaps(
    out_dir,
    combined_table,
    corruptions,
    max_severity,
...
def plot_per_corruption_severity_heatmaps(out_dir, rows, corruptions, max_severity):
    metric_configs = [
        ("mask_AP", "Mask AP", "fig_mask_ap"),
        ("mask_AP50", "Mask AP50", "fig_mask_ap50"),
        ("mask_AP75", "Mask AP75", "fig_mask_ap75"),
    ]

    for corruption in corruptions:
        corruption_title = corr_name(corruption)

        for metric_key, metric_label, prefix in metric_configs:
            table = build_corruption_severity_metric_table(
                rows,
                corruption,
                metric_key,
                max_severity,
            ...
            severity_cols = [f"S{s}" for s in range(1, max_severity + 1)]

            plot_heatmap(
                out_dir,
                f"{prefix}_{corruption}_severity",
                table,
                severity_cols,
                f"{metric_label} under {corruption_title}",
                cmap="coolwarm",
                reverse=False,
                value_label=metric_label,
                vmin=0.0,
            ...
            write_bundle(
                out_dir,
                f"table_{metric_key.lower()}_{corruption}_severity",
                table,
            )
...
def main():
    ...
    rows = collect_eval_rows(results_root, args.run_name)

    combined_table = build_combined_corruption_severity_table(
        rows,
        args.corruptions,
        args.max_severity,
    )
    ...
    plot_combined_corruption_severity_heatmaps(
        out_dir,
        tables["table_combined_clean_corruption_severity_ap_ap50_ap75"],
        args.corruptions,
        args.max_severity,
    )
    plot_per_corruption_severity_heatmaps(
        out_dir,
        rows,
        args.corruptions,
        args.max_severity,
    ...
    report = {
        ...
        "clean_included_in_combined_heatmaps": True,
    }
